...
Status: design of record (draft, awaiting maintainer sign-off)
Date: 2026-06-13
Issue: #223
...
compromise. This trade-off is the key sign-off item below.
...
- **Decision A - default key source.**
  - (a) Auto-generated `0600` key file as the universal default; `MXLRC_MASTER_KEY`
    as an opt-in override. (Recommended: zero-config for native, env-override for
    Docker.)
  - (b) `MXLRC_MASTER_KEY` required (no auto key file at all). Most secure
    key/data separation, but breaks zero-config native use and first-run UX.
  - (c) Auto key file, but **refuse to auto-create it inside `/config`** (Docker
    mode) and instead require `MXLRC_MASTER_KEY` there - i.e. env-key is mandatory
    in Docker, key file is the native default. (Strongest "safe by default"; costs
    a hard failure on first Docker run until the operator sets the env var.)
- **Decision B - AAD binding.** Bind each ciphertext to its `name` via GCM AAD
  (recommended, prevents row-swap) vs no AAD (simpler). Low cost; recommend yes.
- **Decision C - webhook-key model.** Confirm the static configured webhook
  ...
  stay one-way and out of scope. (Recommend: yes, encrypt the static keys; leave
  `api_key_metadata` untouched.)
...
1. **Crypto + store foundation**: `internal/secrets` package (AES-256-GCM
   ...
   store, key-resolution (`MXLRC_MASTER_KEY` > key file, loud failures).
