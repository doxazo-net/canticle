...
# floor stored in scripts/coverage-floor.json. A PR that degrades any
...
#   --floor <path>   Path to the floor JSON file (default: scripts/coverage-floor.json
...
#   After running --lower, commit both scripts/coverage-floor.json and the
...
# Repo root: used to locate go.mod, scripts/coverage-floor.json, and as the
...
if [ -n "$cover_arg" ]; then
  ...
  # Generate a fresh profile in a temp path (the pre-push gate normally supplies
  ...
  (cd "$repo_root" && go test -count=1 -covermode=atomic \
    -coverprofile="$COVER_OUT" $floor_pkgs) || {
